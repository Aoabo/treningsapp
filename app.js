 // ============================================================      
  ...
  const state = {                                           
    ...
    spreadsheetId: localStorage.getItem('spreadsheet_id') || null,
  ...
  // Vis Google Picker så brukeren velger regnearket appen skal bruke
  function pickSpreadsheet() {
    return new Promise((resolve) => {
      if (!window.gapi) {
        alert('Google Picker laster inn – prøv å logge inn på nytt om
  et øyeblikk.');
        resolve(null);
        return;
      }
      gapi.load('picker', { callback: () => {
        const view = new
  google.picker.View(google.picker.ViewId.SPREADSHEETS);
        const picker = new google.picker.PickerBuilder()
          .addView(view)
          .setOAuthToken(state.token)
          .setDeveloperKey(CONFIG.API_KEY)
          .setTitle('Velg treningsregnearket ditt')
          .setCallback((data) => {
  ...
  // Lar brukeren bytte til et annet regneark senere
  ...
  async function sheetsGet(range) {
    const url = `https://sheets.googleapis.com/v4/spreadsheets/${state.
  ...
  async function sheetsAppend(range, values) {
    const url = `https://sheets.googleapis.com/v4/spreadsheets/${state.
  spreadsheetId}/values/${encodeURIComponent(range)}:append?valueInputO
  ption=USER_ENTERED`;
    const res = await fetch(url, {
      method: 'POST',
      headers: {
    ...
    if (!res.ok) {
      if (res.status === 401) { signOut(); return; }
      const data = await res.json();
      throw new Error(data.error?.message || 'Skriving til ark
  feilet');
    }
  }
  ...
  spreadsheetId}:batchUpdate`;
    ...
    if (!res.ok) {
      ...
      if (!data.error?.message?.includes('already exists')) {
        throw new Error('Kunne ikke opprette loggark: ' +
  (data.error?.message || ''));
      }
    }
  }
    ...
    try {
      ...
      if (e.status === 400) {
        throw new Error(
          ...
          '   A: Dato  B: Øvelse-ID  C: Øvelse  D: Sett  E: Reps  F:
  Vekt (kg)  G: Notat\n\n' +
    ...
    if (!rows || rows.length === 0) {
      await sheetsAppend(CONFIG.LOG_SHEET + '!A1', [
        ['Dato', 'Øvelse-ID', 'Øvelse', 'Sett', 'Reps', 'Vekt (kg)',
  'Notat'],
      ]);
    }
  }
  ...
  async function loadExercises() {
    ...
    try {
      ...
      if (e.status === 400) {
        throw new Error(
          'Finner ikke arket "' + CONFIG.EXERCISE_SHEET + '" i
  regnearket ditt.\n\n' +
          'Kjør Apps Script-en "opprettOvelseSheet" først for å
  opprette det.'
        );
      }
      throw e;
  ...
  function renderExercises() {
    const styrke  = state.exercises.filter(e => e.type ===
  'reps_weight' || e.type === 'reps' || e.type === 'rounds');
    ...
    $('content').innerHTML = `
      <div class="exercise-list">
        ...
        <button class="btn-add-exercise" id="btn-add-exercise">+ Ny
  øvelse</button>
      </div>
    `;
  ...
  function exerciseCardHTML(ex) {
    ...
    return `
      <div class="exercise-card ${doneToday ? 'done' : ''}"
  data-id="${ex.id}">
        ...
        <div class="exercise-last">
          ${lastLog ? formatLastLog(lastLog, ex.type) : '<span
  class="no-log">Ikke logget ennå</span>'}
        </div>
      </div>
    `;
  }
    ...
    if (ex.type === 'hold') {
      setsHTML = `
        <div class="hold-message">
          Utfør tøyningen og trykk "Marker som utført" når du er
  ferdig.
        </div>
      `;
    } else {
    ...
    $('content').innerHTML = `
      <div class="log-form">
        ...
        ${ex.description ? `<div
  class="log-description">${ex.description}</div>` : ''}
        ${setsHTML}
        <div class="log-note">
          <label>Notat (valgfritt)</label>
          <input type="text" id="log-note" placeholder="F.eks. føltes
  tungt i dag" />
        </div>
  ...
  function setupLogFormInteractions() {
    ...
    const inputs = Array.from(document.querySelectorAll('.set-row
  input'));
    const noteInput = $('log-note');

    // Autofokus på første reps-felt (bare når det finnes sett å fylle
  ut)
    if (inputs[0]) inputs[0].focus();

    // Enter hopper til neste felt; fra siste input hopper den til
  notat → lagre
  ...
  function setRowHTML(setNum, type, last) {
    ...
    return `
      <div class="set-row ${isWeight ? '' : 'reps-only'}"
  data-set="${setNum}">
        <span class="set-num">${setNum}</span>
        <input type="number" class="set-reps"
          placeholder="${lastReps || (type === 'rounds' ? 'runder' :
  'reps')}"
  ...
  '').replace('.', ',');
        if (reps) rows.push([today, ex.id, ex.name, setNum, reps,
  weight, note]);
      });
    ...
    $('content').innerHTML = `
      <div class="history-view">
        <div class="history-tabs">
          <button class="history-tab ${tab === 'sessions' ? 'active' :
  ''}" data-tab="sessions">Økter</button>
          <button class="history-tab ${tab === 'exercise' ? 'active' :
  ''}" data-tab="exercise">Per øvelse</button>
        </div>
  ...
  function renderSessions() {
    ...
    if (dates.length === 0) {
      container.innerHTML = '<p class="no-data">Ingen økter logget
  ennå.</p>';
      return;
    }
  ...
  exerciseOrder.push(exId); }
      ...
      html += `
        <div class="history-session">
          ...
          ${exerciseOrder.map(id => sessionExerciseHTML(id,
  byExercise[id])).join('')}
        </div>
      `;
    }
  ...
  function renderExerciseFilter() {
    ...
    $('history-exercise').addEventListener('change', e =>
  ...
  function renderHistoryTable(exerciseId) {
    ...
    if (rows.length === 0) {
      container.innerHTML = '<p class="no-data">Ingen data logget ennå
  for denne øvelsen.</p>';
      return;
    }
  ...
  : ''}
          ...
          ${sets[0]?.[6] ? `<div class="history-note">📝
  ${sets[0][6]}</div>` : ''}
        </div>
      `;
    }
  ...
  function formatLastLog(row, type) {
    if (type === 'hold')         return '<span
  class="last-log">Strekk</span>';
    if (type === 'rounds')       return `<span class="last-log">Sist:
  ${row[4] || '–'} runder (sett ${row[3]})</span>`;
    if (type === 'reps_weight')  return `<span class="last-log">Sist:
  ${row[4] || '–'} reps @ ${row[5] ? row[5] + ' kg' : '–'} (sett
  ${row[3]})</span>`;
    return `<span class="last-log">Sist: ${row[4] || '–'} reps (sett
  ${row[3]})</span>`;
  }
    ...
    $('content').innerHTML = `
      <div class="new-exercise-form">
        ...
        <div class="form-row">
          ...
          <input type="text" id="new-name" placeholder="F.eks.
  Sidegange med strikk" />
        ...
        <div class="form-row">
          ...
          <textarea id="new-desc" rows="4" placeholder="Hvordan utføres
   øvelsen"></textarea>
        </div>
  ...
  async function saveNewExercise() {
    ...
    try {
      await sheetsAppend(CONFIG.EXERCISE_SHEET + '!A:D', [[id, name,
  type, desc]]);
  ...
  async function showApp() {
    ...
    $('content').innerHTML = '<div class="loading">Kobler til Google
  Sheets…</div>';
    ...
    try {
      ...
      $('content').innerHTML = `<div class="error">Feil ved lasting:
  ${err.message}</div>`;
    }
  }
